...
export async function handlePostApiKey(request, env) {
  ...
  try {
    const payload = await verifyAuth0JWT(token, env);
    const { label } = await readObject(request);
    if (label != null && (typeof label !== 'string' || label.length > 256)) return Response.json({ error: 'Invalid label' }, { status: 400 });
...
export async function handlePatchApiKey(request, env, id) {
  ...
  try {
    const payload = await verifyAuth0JWT(token, env);
    const { label } = await readObject(request);
    if (typeof label !== 'string' || label.length > 256) return Response.json({ error: 'Invalid label' }, { status: 400 });
    const result = await env.DB.prepare(
      `UPDATE api_keys SET label = ? WHERE id = ? AND user_id = ?`
    ).bind(label, id, payload.sub).run();
    if (!result.meta?.changes) return Response.json({ error: 'API key not found' }, { status: 404 });
